 ...
 */
// #define MAXNODE		100 // ZXK: this is too small!!!
#define MAXNODE		32767 // ZXK: this is too small!!!
...
NODE *create_table_node(char *relname, NODE *attrlist)
{
    ...
    char *primaryKeyAttr = nullptr;
    NODE *curr, *iter;
    for (iter = attrlist; iter != nullptr; iter = iter->u.LIST.next) {
        curr = iter->u.LIST.curr;
        if (curr->kind == N_ATTRPROP) {
            if (curr->u.ATTRPROP.attrProp == PRIMARY_KEY) {
                primaryKeyAttr = curr->u.ATTRPROP.attrname;
            }
        }
    }

    n -> u.CREATETABLE.primary = primaryKeyAttr;
    return n;
}
 ...
 */
NODE *insert_node(char *relname, NODE *tuplelist)
{
    NODE *n = newnode(N_INSERT);

    n->u.INSERT.relname = relname;
    n->u.INSERT.tuplelist = tuplelist;
    return n;
}
...
NODE *condition_node(NODE *lhsRelattr, CompOp op, NODE *rhsRelattrOrValue)
{
    ...
    if (rhsRelattrOrValue != nullptr) {
        ...
        n->u.CONDITION.rhsRelattr = nullptr;
        n->u.CONDITION.rhsValue = nullptr;
    }
    return n;
}
...
NODE *attrtype_node(char *attrname, char *type, int length, int nullable)
{
    ...
    n -> u.ATTRTYPE.length = length + 1;
    n -> u.ATTRTYPE.nullable = nullable;
    return n;
}

/* zxk: add support of rel propertise */
NODE *attrprop_node(AttrProp prop, char *attrname)
{
    NODE *n = newnode(N_ATTRPROP);

    n->u.ATTRPROP.attrProp = prop;
    n->u.ATTRPROP.attrname = attrname;
    return n;
}
